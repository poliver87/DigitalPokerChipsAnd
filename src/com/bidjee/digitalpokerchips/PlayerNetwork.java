	...
	public void parseGameMessage(String msg) {
		if (!tableConnected) {
			...
			Gdx.app.postRunnable(new Runnable() {
				@Override
				public void run() {
					player.sendChips(ChipStack.parseStack(chipString));
				}
			});
		...
		} else if (msg.contains(HostNetwork.TAG_DEAL_OPEN)&&msg.contains(HostNetwork.TAG_DEAL_CLOSE)) {
			int startIndex = msg.indexOf(HostNetwork.TAG_DEAL_OPEN) + HostNetwork.TAG_DEAL_OPEN.length();
			int endIndex = msg.indexOf(HostNetwork.TAG_DEAL_CLOSE);
			final int dealStage = Integer.parseInt(msg.substring(startIndex, endIndex));
			Gdx.app.postRunnable(new Runnable() {
				@Override
				public void run() {
					player.promptDeal(dealStage);
				}
			});
		} else if (msg.contains(HostNetwork.TAG_DEAL_WAIT_OPEN)&&msg.contains(HostNetwork.TAG_DEAL_WAIT_CLOSE)) {
			int startIndex = msg.indexOf(HostNetwork.TAG_DEAL_WAIT_NAME_OPEN) + HostNetwork.TAG_DEAL_WAIT_NAME_OPEN.length();
			int endIndex = msg.indexOf(HostNetwork.TAG_DEAL_WAIT_NAME_CLOSE);
			final String dealerName = msg.substring(startIndex, endIndex);
			startIndex = msg.indexOf(HostNetwork.TAG_DEAL_WAIT_STAGE_OPEN) + HostNetwork.TAG_DEAL_WAIT_STAGE_OPEN.length();
			endIndex = msg.indexOf(HostNetwork.TAG_DEAL_WAIT_STAGE_CLOSE);
			final int dealStage = Integer.parseInt(msg.substring(startIndex, endIndex));
			Gdx.app.postRunnable(new Runnable() {
				@Override
				public void run() {
					player.notifyWaitDealer(dealerName,dealStage);
				}
			});
		} else if (msg.contains(HostNetwork.TAG_BET_WAIT_OPEN)&&msg.contains(HostNetwork.TAG_BET_WAIT_CLOSE)) {
			int startIndex = msg.indexOf(HostNetwork.TAG_BET_WAIT_NAME_OPEN) + HostNetwork.TAG_BET_WAIT_NAME_OPEN.length();
			int endIndex = msg.indexOf(HostNetwork.TAG_BET_WAIT_NAME_CLOSE);
			final String betterName = msg.substring(startIndex, endIndex);
			Gdx.app.postRunnable(new Runnable() {
				@Override
				public void run() {
					player.notifyWaitBet(betterName);
				}
			});
		} else if (msg.contains(HostNetwork.TAG_SHOW_CARDS)) {
			Gdx.app.postRunnable(new Runnable() {
				@Override
				public void run() {
					player.promptShowCards();
				}
			});
